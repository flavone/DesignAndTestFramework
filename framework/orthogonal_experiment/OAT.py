...
def auto_fill(data):
    """
    自动补齐因子列，如果因子列不对齐，则存在正交结果有空值的情况；补齐策略是从左到右顺序补充到最大长度

    :param data: 原OrderedDict

    :return: 补齐的OrderedDict

    """
    filled_data = []
    for k in data.items():
        filled_data.append(k)
    max_len = max([len(v[1]) for v in filled_data])

    for v in filled_data:
        vv = v[1]
        index = 0
        while len(v[1]) < max_len:
            v[1].append(vv[index])
            index += 1
    new_data = OrderedDict(filled_data)
    return new_data
...
class OAT(object):
    ...
    def genSets(self, params, fill=1, mode=0, num=1):
        """
