...
class HttpUtil:
    ...
    def get_json(self, relative_url: str, para=None):
        ...
        if relative_url.find('%s') >= 0:
            ...
            resp = self.session.request(method='GET', url=self.url + relative_url, params=None)
        else:
            resp = self.session.request(method='GET', url=self.url + relative_url, params=para)
    ...
    def post_json(self, relative_url, para=None):
        ...
        :param para: 请求参数，只可以是json字符串

        :return: 响应结果的json
        """
        if self.session is None:
            return None
        resp = self.session.request(method='POST', url=self.url + relative_url, json=para)
        if resp.status_code != 200:
            return None
        return resp.json()